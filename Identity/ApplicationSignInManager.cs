...
namespace Projects.Identity
{
    ...
    public class ApplicationSignInManager : SignInManager<ApplicationUser>
    {
        ...
        public string GenerateJwtToken(ApplicationUser user, string? tokenType = null)
        {
            ...
            var token = new JwtSecurityToken(
                ...
                claims: claims,
                expires: tokenType == "Refresh" ?  DateTime.Now.AddDays(15) : DateTime.Now.AddMinutes(15),
                signingCredentials: creds);

            return new JwtSecurityTokenHandler().WriteToken(token);
        }

        public TokenDto GenerateJwtTokens(ApplicationUser user)
        {
            return new TokenDto(
        GenerateJwtToken(user),
        GenerateJwtToken(user, "Refresh")
    );
        }
    }
}
